...
const debug = require('debug')('camelz:cli:util');
...
exports.extractTemplateLambdaLayerDependencies = (template) => {
  ...
  for (const logicalId of Object.keys(templateResources)) {
    ...
    //   - Deployment package directory: $CAMELZ_FUNCTIONS/${LayerName}Layer/
...
exports.extractTemplateLambdaFunctionDependencies = (template) => {
  ...
  for (const logicalId of Object.keys(templateResources)) {
    ...
    //   - Deployment package directory: $CAMELZ_FUNCTIONS/${FunctionName}/
    // - Exceptions will be thrown if these conventions are not followed.
...
exports.formatParameters = (parameters, maxValueLength = 256) => {
...
exports.mergeCommandCapabilities = (templateCapabilities,
                                    commandCapabilities, overrideCapabilities) => {
  debug(`util.mergeCommandCapabilities()`);

  let mergedCommandCapabilities = [];

  debug('- Template');
  for (const capability of templateCapabilities) {
    debug(`  - ${capability}`);
    mergedCommandCapabilities.push(capability);
  }

  debug('- Command');
  for (const capability of commandCapabilities) {
    const i = mergedCommandCapabilities.findIndex(c => c == capability);
    if (i >= 0) {
      debug(`  - ${capability} (matched)`);
    }
    else {
  ...
  debug('- Override');
  for (const capability of overrideCapabilities) {
    const i = mergedCommandCapabilities.findIndex(c => c == capability);
    if (i >= 0) {
      debug(`  - ${capability} (matched)`);
    }
  ...
  return mergedCommandCapabilities;
}
...
exports.formatCapabilities = (capabilities) => {
  ...
  for (const capability of capabilities) {
    formattedCapabilities += `--capability ${capability} \\\n`;
  }
