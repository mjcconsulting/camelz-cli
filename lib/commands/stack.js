...
const formatStackCommand = (config, params, detailed = false) => {
  ...
  let formattedCommand = `camelz ${config.program.name} ${config.command.name} `;
  ...
  const formattedTemplate = (config.current.stack && config.current.stack.template) ? `--template-name ${config.current.stack.template.name} ` : '';
...
const checkStackPrerequisites = async (config) => {
  ...
  if (config.current.stack.parameters && config.current.stack.parameters.length > 0) {
    const requiredStacks = config.current.stack.parameters.filter(p => p.ParameterKey.endsWith('StackName') && p.ParameterValue != '')
      ...
      do {
        ...
        if (status != 'CREATE_COMPLETE') {
          if (config.current.flags.monitor) {
            console.info(chalk.bold(`\rStatus: ${status}`));
