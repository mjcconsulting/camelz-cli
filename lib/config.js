...
const debug = require('debug')('camelz:cli:config');
...
exports.saveDescribeTemplatesCommandOptions = (options) => {
  debug(`config.saveDescribeTemplatesCommandOptions()` + ((options.templateName) ? ` [Template: ${options.templateName}]` : ''));
...
exports.saveValidateTemplateCommandOptions = (options) => {
  debug(`config.saveValidateTemplateCommandOptions() [Template: ${options.templateName}]`);
...
exports.loadUserData = () => {
  ...
  const configPath = path.join(os.homedir(), '.camelz');
  ...
  // Currently these are stored in each user's ~/.camelz directory, which is meant as a quick but temporary solution
...
exports.loadStackData = () => {
  ...
  if (exports.current.stack) {
    ...
    const configPath = path.join(exports.current.config.path,
                                 ...
                                 'Accounts', exports.current.account.Name + '[' + exports.current.account.Id + ']',
                                 'Regions', exports.current.region.Name,
                                 'Stacks', exports.current.stack.name); // This often won't exist!

    exports.stack = {
      flags:        util.getData(util.getDataBody(path.join(configPath, 'Flags.json')), schemas.flags),
      parameters:   util.getParameters(util.getParametersBody(path.join(configPath, 'Parameters.json')), schemas.parameters),
      tags:         util.getData(util.getDataBody(path.join(configPath, 'Tags.json')), schemas.tags)
    };
  }
  else {
...
exports.loadTemplateData = () => {
  debug(`config.loadTemplateData()`);

  if (exports.current.stack && exports.current.stack.template && exports.current.stack.template.body) {
    debug(`- Load template configuration data`);
    // ToDo: This load works a bit differently in that we've already loaded the templateBody in an earlier method
    //       At some point, we may want to keep Templates only in an S3 Bucket and not on local disk, so we may need
    //       to have logic to extract information from an URL, not the Body.
    const template = util.getTemplate(exports.current.stack.template.body);
    exports.template = {
      parameters:   util.extractTemplateParameters(template),
      capabilities: util.extractTemplateCapabilities(template),
      layerDependencies: util.extractTemplateLambdaLayerDependencies(template),
      functionDependencies: util.extractTemplateLambdaFunctionDependencies(template)
    };
  }
  else {
...
exports.setCurrentStackParameters = (parameters = []) => {
  ...
  // Note this merges all but calculatedParameters, but those aren't known until after Lambda deployment packages
  // are created and uploaded, where new objectVersions might be created. As a result, a final merge of such
  // calculatedParameters must be done inside the createStack method, during the deployment package upload process.
...
exports.updateCurrentStackParameters = (parameters = []) => {
  debug(`config.updateCurrentStackParameters()`);

  util.replaceCalculatedParameters(exports.current.stack.parameters, parameters);
}
...
exports.setCurrentStackCapabilities = (capabilities = []) => {
  debug(`config.setCurrentStackCapabilities()`);

  exports.current.stack.capabilities = util.mergeCommandCapabilities(exports.template.capabilities,
                                                                     exports.command.capabilities, capabilities);
}

exports.setCurrentStackDependencies = () => {
  debug(`config.setCurrentStackDependencies()`);

  exports.current.stack.layerDependencies = exports.template.layerDependencies;
  exports.current.stack.functionDependencies = exports.template.functionDependencies;
}
